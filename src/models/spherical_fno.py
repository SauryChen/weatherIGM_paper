...
class SphericalFourierNeuralOperatorNet(nn.Module):
    ...
    def forward(self,
        ...
        x = torch.cat((x, x_cons, time_embedding), dim=1)
        ...
        if metrics == None:
            ...
            loss = [m(x, y, self.traced_vars, lat) for m in metrics]
        return loss, x


    def evaluate(self, x, x_cons, time_embedding, y, out_variables, transform, metrics, lat, clim, log_postfix):
        _, preds = self.forward(x, x_cons, time_embedding, y, metrics=None, lat=lat)
...
if __name__ == "__main__":
    ...
    loss, preds = model(x = rand_input, x_cons = rand_const, time_embedding = rand_time, y = rand_output,
                        metrics = None, lat = None)
