...
class GraphCastNet(Module):
    ...
    # the original input only has grid_nfeatures as input,
    # since we need to change the time-embedding to be time-dependent in iteration evaluation,
    # we need to add grid_nconst and grid_ntime as input, and preprocess the n_time fisrt.
    def forward(
        self,
        grid_nfeat: Tensor,
        grid_nconst: Tensor,
        ...
        self.to(device=grid_nfeat.device)

        B, _, H, W = grid_nfeat.shape
        time_embedding = -torch.cos(2* np.pi * grid_ntime / 8760)
        time_embedding = time_embedding.reshape(B, 1, 1, 1).repeat(1, 1, H, W)
        grid_nfeat = torch.cat((grid_nfeat, grid_nconst, time_embedding), dim=1)
        ...
        if metrics == None:
            ...
            loss = [m(outvar, grid_y, self.traced_vars, lat) for m in metrics]

        return loss, outvar
    
    def evaluate(self, grid_nfeat, grid_nconst, grid_ntime, grid_y, out_variables, transform, metrics, lat, clim, log_postfix):
        _, preds = self.forward(grid_nfeat, grid_nconst, grid_ntime, grid_y, metrics=None, lat=lat)
        return [m(preds, grid_y, transform, out_variables, lat, clim, log_postfix) for m in metrics]
    
    def evaluate_iter(self, grid_nfeat, grid_nconst, grid_ntime, grid_y, metrics, lat):
        with torch.no_grad():
            _, preds = self.forward(grid_nfeat, grid_nconst, grid_ntime, grid_y, metrics=None, lat=lat)
        return preds
...
if __name__ == "__main__":
    ...
    model = GraphCastNet(
        mesh_level = 5,
        input_res = (121, 240),
        input_dim_grid_nodes = 39*3+6, # 123
        output_dim_grid_nodes = 39,
    ...
    rand_const = torch.rand((1, 5, 121, 240)).to(device)
    rand_time = torch.rand((1)).to(device)
    rand_output = torch.rand((1, 39, 121, 240)).to(device)

    # with torch.no_grad():
    #     loss, preds = model(grid_nfeat = rand_input, grid_nconst = rand_const, grid_ntime = rand_time,
    #                          grid_y = rand_output, metrics = None, lat = None)
    loss, preds = model(grid_nfeat = rand_input, grid_nconst = rand_const, grid_ntime = rand_time,
                        grid_y = rand_output, metrics = None, lat = None)
